...
@section('content')
    <div class="stats-card">
        <div class="stats-header">
            <h3>Edit Log Bimbingan</h3>
        </div>
    </div>

    <form action="{{ route('student.final-project.guidance.update', $log->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="form-card">
            ...
            <div class="form-group">
                ...
                <select name="supervisor_id" class="form-control" required>
                    ...
                    @if($finalProject->supervisor1)
                        <option value="{{ $finalProject->supervisor1->id }}" {{ $log->supervisor_id == $finalProject->supervisor1->id ? 'selected' : '' }}>
                            Pembimbing 1 - {{ $finalProject->supervisor1->name }}
                        </option>
                    @endif
                    @if($finalProject->supervisor2)
                        <option value="{{ $finalProject->supervisor2->id }}" {{ $log->supervisor_id == $finalProject->supervisor2->id ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <input type="date" name="guidance_date" class="form-control" value="{{ old('guidance_date', $log->guidance_date->format('Y-m-d')) }}" required>
                @error('guidance_date')<span class="error">{{ $message }}</span>@enderror
            </div>

            <div class="form-group">
                <label>Materi yang Dibimbing *</label>
                <textarea name="materials_discussed" class="form-control" rows="4" required>{{ old('materials_discussed', $log->materials_discussed) }}</textarea>
                @error('materials_discussed')<span class="error">{{ $message }}</span>@enderror
            </div>

            <div class="form-group">
                <label>Catatan Mahasiswa (Opsional)</label>
                <textarea name="student_notes" class="form-control" rows="3">{{ old('student_notes', $log->student_notes) }}</textarea>

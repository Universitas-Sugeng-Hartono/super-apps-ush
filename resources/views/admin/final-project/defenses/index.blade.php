...
@section('content')
    <div class="content-card">
        <div class="card-header">
            ...
            <div class="filter-toolbar">
                <form action="{{ route('admin.final-project.defenses.index') }}" method="GET" class="filter-form">
                ...
                <a href="{{ route('admin.final-project.proposals.bulk-zip', ['prodi' => $prodiFilter ?? null, 'type' => 'final']) }}" class="btn-download-zip">
                ...
                <div class="approval-card">
                    <div class="approval-header">
                        <div class="student-section">
                            <div style="display: flex; align-items: center; flex-wrap: wrap; gap: 5px;">
                                <h4 style="margin: 0;">{{ $defense->finalProject->student->nama_lengkap }}</h4>
                                @php
                                    $docs = $defense->finalProject->documents->where('document_type', 'final');
                            ...
                            <p class="meta">
                                <span><i class="bi bi-card-text"></i> {{ $defense->finalProject->student->nim }}</span>
                                <span><i class="bi bi-calendar3"></i> Daftar: {{ $defense->registered_at->format('d M Y') }}</span>
                            </p>
                        </div>
                        <div class="action-buttons">
                            <button type="button" class="btn-approve" onclick="showApproveModal({{ $defense->id }})">
                                    <i class="bi bi-check-circle"></i> Setujui
                                </button>
                            <button type="button" class="btn-reject" onclick="showRejectModal({{ $defense->id }})">
                    ...
                    <div class="proposal-details">
                        <div class="detail-row">
                            ...
                            <p style="font-weight: 600; color: #333;">{{ $defense->finalProject->title ?? '-' }}</p>
                        </div>
                        @if($defense->finalProject->title_en)
                        <div class="detail-row">
                            <strong>Judul TA (Bahasa Inggris):</strong>
                            <p style="font-style: italic; color: #666;">{{ $defense->finalProject->title_en }}</p>
                        </div>
                        @endif
                        <div class="detail-row">
                            <strong>Pembimbing:</strong>
                            <p>
                                {{ $defense->finalProject->supervisor1->name ?? '-' }}
                                @if($defense->finalProject->supervisor2)
                                    , {{ $defense->finalProject->supervisor2->name }}
                                @endif
                            </p>
                        </div>
                        <div class="detail-row">
                            <strong>Jumlah Bimbingan:</strong>
                            <p>{{ $defense->finalProject->guidanceLogs()->approved()->count() }} kali bimbingan</p>
                        </div>
                    </div>

                     @php
                        $defenseDocs = $defense->finalProject->documents
    ...
    <div id="bulkApproveModal" class="modal" style="display: none;">
        <div class="modal-content" style="max-width: 800px; width: 100%;">
            <h4>Approve Masal Pendaftaran Sidang</h4>
            ...
            <form id="bulkApproveForm" method="POST" action="{{ route('admin.final-project.defenses.approve-all') }}">
                @csrf
                        ...
                        <thead style="background: #f5f5f5; position: sticky; top: 0; z-index: 1;">
                            <tr>
                                <th style="padding: 12px; border-bottom: 2px solid #ddd; text-align: center; width: 50px;">
                                    <input type="checkbox" id="checkAllDefenses" onclick="toggleAllCheckboxes(this, 'defense-checkbox')" checked>
                                </th>
                                <th style="padding: 12px; border-bottom: 2px solid #ddd; text-align: left;">Nama & Prodi</th>
                                @if(in_array(($role ?? null), ['superadmin', 'masteradmin'], true))
                                <th style="padding: 12px; border-bottom: 2px solid #ddd; text-align: left; width: 250px;">Jadwal Sidang (Opsional)</th>
                                @endif
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr style="border-bottom: 1px solid #eee;">
                                <td style="padding: 12px; text-align: center;">
                                    <input type="checkbox" class="defense-checkbox" name="selected_ids[]" value="{{ $defense->id }}" checked>
                                </td>
                                <td style="padding: 12px;">
                                    <div style="font-weight: 600; color: #333;">{{ $defense->finalProject->student->nama_lengkap }}</div>
                                    <div style="font-size: 13px; color: #666; margin-top: 4px;">{{ $defense->finalProject->student->program_studi }} - {{ $defense->finalProject->student->nim }}</div>
                                </td>
                                @if(in_array(($role ?? null), ['superadmin', 'masteradmin'], true))
                                <td style="padding: 12px;">
                                    <input type="datetime-local" name="scheduled_dates[{{ $defense->id }}]" class="form-control" style="width: 100%; padding: 8px; font-size: 13px;">
                                </td>
                                @endif
                            </tr>
                            @empty
...
function showApproveModal(id) {
    document.getElementById('approveForm').action = `/admin/final-project/defenses/${id}/approve`;
    document.getElementById('approveModal').style.display = 'flex';
}

function showRejectModal(id) {
    document.getElementById('rejectForm').action = `/admin/final-project/defenses/${id}/reject`;
